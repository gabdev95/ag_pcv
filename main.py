...
pontos_cidades = ["C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9", "C10", "C11", "C12", "C13", "C14", "C15", 
...
class Solucao:
    def __init__(self, cidades):
        ...
        self.full_rota = cidades + [cidades[0]]  # Adiciona a primeira cidade no final
        self.distancia = self.calcula_distancia()
        
    def calcula_distancia(self):
        distancia = 0
        for i in range(len(self.full_rota) - 1):
            distancia += distancias[self.full_rota[i]][self.full_rota[i+1]]
        return distancia
...
def classifica_populacao(populacao):
    populacao_classificada = sorted(populacao, key=lambda solucao: solucao.distancia)
...
def cruzamento(pai1, pai2):
    filho_cidade = [None] * len(pai1.cidades)
    inicio = random.randint(0, len(pai1.cidades) - 1)
    fim = random.randint(inicio, len(pai1.cidades) - 1)
    for i in range(inicio, fim + 1):
        filho_cidade[i] = pai1.cidades[i]
    for cidade in pai2.cidades:
        if cidade not in filho_cidade:
            for i in range(len(filho_cidade)):
                if filho_cidade[i] is None:
                    filho_cidade[i] = cidade
                    break
    child = Solucao(filho_cidade)
    return child
...
def algoritmo_genetico():
    ...
    for i in range(GERACOES):
        ...
        if populacao[0].distancia < melhor_distancia:
            ...
            melhor_distancia = populacao[0].distancia
            melhor_distancia_total = " -> ".join(pontos_cidades[i] for i in melhor_solucao.full_rota)  # Transforma os números em nomes de cidades
            print(f"Iteração {i+1} - Aptidão: {melhor_distancia}km \nPercurso: {melhor_distancia_total}\n ")
    melhor_distancia_total = " -> ".join(pontos_cidades[i] for i in melhor_solucao.full_rota)  # Transforma os números em nomes de cidades
    print('=' * 150)
    print('MELHOR SOLUÇÃO ENCONTRADA'.center(150))
    print('=' * 150)
